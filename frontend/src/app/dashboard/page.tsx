...
import { Mail, CheckCircle, Shield, History, EyeOff, XCircle, Clock, CalendarDays, Rocket, Info, ChevronDown, Loader2, LogOut } from 'lucide-react';
...
const DashboardPage: NextPage = () => {
  ...
  const handleScanServices = async () => {
    setIsScanning(true);
    try {
      const res = await fetch('http://localhost:5000/api/mail/scan', {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const handleUnsubscribe = async (serviceId: string) => {
    ...
    try {
      ...
      setUser(prevUser => {
        ...
        return { ...prevUser, services: updatedServices, unsubscribedCount: prevUser.unsubscribedCount + 1 };
      });
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-green-50 min-h-screen text-gray-900 p-6 sm:p-8 md:p-12">
      {/* Dashboard Header */}
      <motion.div
      ...
      >
        ...
        >
          ...
          >
            {isScanning ? (
              <>
                <Loader2 className="animate-spin mr-3 h-6 w-6" /> Scanning...
              </>
            ) : (
              <>
